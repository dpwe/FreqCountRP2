...
void FreqCountRP2::beginExtTrig(uint freq_pin, uint trig_pin) {
    ...
    gpio_init(mTriggerPin); 
    gpio_set_dir(mTriggerPin, GPIO_IN);
    irq_set_exclusive_handler(IO_IRQ_BANK0, _on_trigger);
    gpio_set_irq_enabled(mTriggerPin, GPIO_IRQ_EDGE_RISE, true);
...
void FreqCountRP2::beginTimer(uint freq_pin, uint period_ms) {
    ...
    add_repeating_timer_ms(-period_ms, repeating_timer_callback, NULL, &mTimer);
}
...
void _FreqCountRP2::end()
{
    ...
    if(mTriggerPin == 0) {
        ...
        gpio_set_irq_enabled(mTriggerPin, GPIO_IRQ_EDGE_RISE, false);
